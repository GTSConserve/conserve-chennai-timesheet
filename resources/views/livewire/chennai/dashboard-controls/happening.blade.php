        ...
        <div id="msform">
            <fieldset class="mt-3">
                <table id="zero-config1" class="table" style="width:100%;">
                    ...
                    <tbody>
                        @foreach ($heappnings as $heappning)
                            <tr class="text-center">
                                <td>{{$loop->index+1}}</td>
                                <td>{{$heappning->date}}</td>
                                <td>{{$heappning->title}}</td>
                                <td><input type="checkbox" wire:click="status({{$heappning->id}},)" @if ($heappning->status == 1) checked @endif></td>
                                <td><svg xmlns="http://www.w3.org/2000/svg" wire:click="delete({{$heappning->id}})" width="20" color="" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></td>
                            </tr>
                        @endforeach
                    </tbody>
            </fieldset>
